...
public:
    ...
    std::vector<int>& queries) {
        ...
        sort(items.begin(), items.end(), [](std::vector<int> a, std::vector<int> b){
            return a[0] < b[0];
        });
        std::vector<std::vector<int>>::iterator itr = items.begin(), prev = itr;
        std::vector<std::pair<int, int>> reducedItems;
        std::vector<std::pair<int, int>>::iterator upperLimit;
        pair<int,int> queryAux;

        // Remove unuseful
//        printV(items.begin(), items.end());
        reducedItems.push_back(std::pair<int, int>((*itr)[0],(*itr)[1]));
...
//        for(auto item : reducedItems) printf("%i,%i ", item.first, item.second);
//        printf("\n");

        // Do queries
        for (int query : queries) {
//            printf("%i -> ", query);
            queryAux.first = query;
            queryAux.second = 0;
            upperLimit = upper_bound(reducedItems.begin(), reducedItems.end(), queryAux,
            [](const std::pair<int, int> a, const std::pair<int, int> b){
                return a.first < b.first;
            });

            solution.push_back(
                (upperLimit != reducedItems.begin())?
                    reducedItems[distance(reducedItems.begin(), upperLimit)-1].second:
                    0
            );
        }
        return solution;
