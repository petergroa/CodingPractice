...
class Solution {
public:
    static int intCeil(int x, int y) {
        if (x <= 0) throw std::invalid_argument("Received an negative number");
        return 1 + ((x-1) / y);
    }

    static bool possible(int &n, std::vector<int>& quantities, int& commonDiv) {
        int totalPlaces = 0;
        for (int product : quantities) {
            totalPlaces += intCeil(product, commonDiv);
        }
        return totalPlaces > n;
    }
    ...
    static int minimizedMaximum(int n, vector<int>& quantities) {
        ...
        while (left < right) {
            mid = (left+right)/2;
            if (possible(n, quantities, mid)) left = mid+1;
            else right = mid;
        }
        return left;
    }
