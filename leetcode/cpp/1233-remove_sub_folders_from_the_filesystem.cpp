 ...
 * 
 * If a folder[i] is located within another folder[j], it is called a subfolder
...
bool pathFamilyComparison(const std::string &a, const std::string &b) {
    if (a==b) return false;
...
std::vector<std::string> removeSubFolders(std::vector<std::string> &folder) {
    ...
    std::vector<std::string>::iterator back = folder.begin(), front = back+1;
    std::vector<std::string> res;

    res.push_back(*back);
    for ( ; front != folder.end(); front++) {
//        printf("%s -> %s? ", back->c_str(), front->c_str());
        if (!front->starts_with(*back+"/")) {
//            printf("add\n");
            back = front;
            res.push_back(*front);
        }
